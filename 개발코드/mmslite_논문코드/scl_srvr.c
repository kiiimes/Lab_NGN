...
/* --------  ---  ------   -------------------------------------------  */
/* 03/14/14  JRB  Add GOOSE publishers if GOOSE_TX_SUPP defined.        */
/*                Init and use "GoEna" to control GOOSE publishers.     */
/*                Init new "options.buf_size".                          */
/*                Exit if scl2_vmd_create_all fails.                    */
/*                Move scl_log_all call to main (for any parse mode).   */
/*                u_mvl_fopen_ind: fix error resp for 61850 FtN1 test.  */
/*                Move iecGooseUnSubscribeAll before clnp_end.          */
/* 08/19/13  JRB           Merge MVLU_RPT_CLIENT into MVLU_RPT_CTRL.    */
...
#include "subnet.h"
#endif
#if defined(GOOSE_RX_SUPP) || defined(GOOSE_TX_SUPP)
...
#endif  /* DEBUG_SISCO */
#endif  /* defined(GOOSE_RX_SUPP) || defined(GOOSE_TX_SUPP) */
...
int main (int argc, char *argv[])
  ...
  ST_FLOAT *MMDC_w, *MMDC_a, *MMDC_v; //solar
  ST_FLOAT *MMET_et, *MMET_eh, *MMET_hws; //windmil
  RUNTIME_TYPE *InstVarRtType;
...
ST_DOUBLE TimeNew, TimeOld;
ST_LONG TimeElapsed;
ST_DOUBLE lastGooseTime = 0;
#endif  /* GOOSE_TX_SUPP     */
  ...
  if (scl_parse_mode == SCL_PARSE_MODE_SCD)
    ...
    if (ret == SX_ERR_CONVERT)
      {
      printf ("WARNING: Non-critical error in SCL file.\n");
      }
    else if (ret)
      {
  ...
  if (ret == SD_SUCCESS)
    {
    SCL_SERVER *scl_server;
    /* Find the server configured in "startup.cfg".             */
    /* Create and enable all GOOSE Publishers for that Server.  */
    for (scl_server = (SCL_SERVER *) list_find_last ((DBL_LNK *) scl_info.serverHead);
      ...
      {
      if (strcmp (scl_server->iedName, startup_cfg.ied_name) == 0 &&
          strcmp (scl_server->apName, startup_cfg.access_point_name) == 0)
        {   /* found the server */
        ret = iecGoosePubCreateAll (&scl_info, scl_server, &mvl_vmd);
        if (ret)
          printf ("iecGoosePubCreateAll failed\n");
        else
          {
          IEC_GOOSE_SEND_USER_INFO *goosehandle;
          /* Enable all GOOSE publishers  */
          for (goosehandle = pub_ctrl.pub_list;
               goosehandle != NULL;
               goosehandle = (IEC_GOOSE_SEND_USER_INFO *) list_get_next (pub_ctrl.pub_list, goosehandle))
            {
            /* GoEnaPtr initialized by iecGoosePubCreateAll. */
            if (goosehandle->GoEnaPtr)
              *goosehandle->GoEnaPtr = SD_TRUE; /* enable this GOOSE publisher  */
            }
          }
        }
      }
    }
#endif  /* defined(GOOSE_TX_SUPP) */
  ...
  if (scl_server)
    scl_svcb = scl_svcb_find (scl_server, "Solar1", "LLN0",  /* always LLN0*/
                              "EMS1SolarRCB",          /* find this SVCB       */
  ...
  baseVarName.object_tag = DOM_SPEC;
  baseVarName.domain_id = "EMS1Solar1";
  baseVarName.obj_name.item_id = "MMDC1"; ///sequence해당 인스턴스 명
  var_with_ms = mvl_vmd_find_var(&mvl_vmd, &baseVarName, NULL);
  MMDC_w = mvlu_get_leaf_data_ptr(var_with_ms, "MX$Watt$mag$f", &InstVarRtType);
  MMDC_a = mvlu_get_leaf_data_ptr(var_with_ms, "MX$Amp$mag$f", &InstVarRtType);
  MMDC_v = mvlu_get_leaf_data_ptr(var_with_ms, "MX$Vol$mag$f", &InstVarRtType);

  baseVarName.obj_name.item_id = "MMET1"; ///sequence해당 인스턴스 명
  var_with_ms = mvl_vmd_find_var(&mvl_vmd, &baseVarName, NULL);
  MMET_et = mvlu_get_leaf_data_ptr(var_with_ms, "MX$EnvTmp$mag$f", &InstVarRtType);
  MMET_eh = mvlu_get_leaf_data_ptr(var_with_ms, "MX$EnvHum$mag$f", &InstVarRtType);
  MMET_hws = mvlu_get_leaf_data_ptr(var_with_ms, "MX$HorWdSpd$mag$f", &InstVarRtType);
  ...
  while (doIt)
    {
	  *MMDC_w = sin((i%720))+1;
	  *MMDC_a = sin((i % 720) + 30)+1;
	  *MMDC_v = sin((i % 720) + 45)+170;

	  *MMET_et = sin((i % 720) + 60)+18;
	  *MMET_eh = sin((i % 720) + 75)+87;
	  *MMET_hws = sin((i % 720) + 90)+1;
	  i += 1;
...
#if defined(GOOSE_TX_SUPP)
    /* Periodically (every 10 seconds) publish all configured GOOSE.  */
    TimeNew = sGetMsTime();
    if (TimeNew > lastGooseTime + 10000)  /* 10000 msec = 10 sec  */
      {
      IEC_GOOSE_SEND_USER_INFO *goosehandle;
      lastGooseTime = TimeNew;
      /* Publish all GOOSE configured in SCL file.      */
      for (goosehandle = pub_ctrl.pub_list;
           goosehandle != NULL;
           goosehandle = (IEC_GOOSE_SEND_USER_INFO *) list_get_next (pub_ctrl.pub_list, goosehandle))
        {
        /* Check if this GOOSE is enabled.  */
        if (goosehandle->GoEnaPtr && *goosehandle->GoEnaPtr != 0)
          {
          /* Update the GOOSE time stamp  */
          goosehandle->gptr->utcTime.secs = (ST_UINT32) time(NULL);
          goosehandle->gptr->utcTime.fraction = 0;
          goosehandle->gptr->utcTime.qflags = 0x0;

          iecGoosePublish (goosehandle);
          }
        }
      }
    /* Retransmit goose messages as appropriate*/
    /* NOTE: TimeNew was set above. */
  ...
  /* NOTE: must be before clnp_end or clnp_snet_term because it calls "clnp_snet_*".*/
  if (iecGooseUnSubscribeAll())
    printf ("Error unsubscribing from all GOOSE messages\n");
#endif

#if defined(GOOSE_TX_SUPP)
  /* If we published any GOOSE, must stop publishing now.     */
  iecGoosePubDestroyAll();
#endif
...
ST_RET all_obj_create (ST_CHAR *startup_cfg_filename,
  ...
  if (ret == SD_SUCCESS)
    {
    options.buf_size = 100000;
    ret = scl_parse_cid (startup_cfg.scl_filename,
  ...
  if (ret == SX_ERR_CONVERT)
    {
    printf ("Non-critical error in SCL file. Check log file for details.\n");
    ret = SD_SUCCESS; /* reset so code continues. */
    }
...
ST_VOID u_mvl_fopen_ind (MVL_IND_PEND *indCtrl)
  ...
  if (fp == NULL)
    ...
    if (errno == ENOENT)
      ...
      _mplas_err_resp (indCtrl, 11, 7);         /* File-non-existent    */
    else
...
static ST_VOID mvl61850_rpt_data_init (ST_VOID)
  ...
  for (rptCtrl = mvl61850_rpt_ctrl_list;
    ...
    /* does what we need.                                               */
    mvlu_integrity_scan_read (rptCtrl,
