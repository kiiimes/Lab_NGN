...
//은숙
tree* getServerDirectory(tree* root,MVL_NET_INFO *clientNetInfo);
tree* getLogicalDeviceDirectory(tree* root,MVL_NET_INFO *clientNetInfo);
tree* getLogicalNodeDirectory(tree* root,MVL_NET_INFO *clientNetInfo);
tree* getReportControlBlock(tree* root);

tree* getNode(char* name,int value) {
	int len=0;
...
tree* insertSibling(tree* parent) {
	int data, sib, son;
	tree* root;
	tree* node;
	tree* tmp;
...
ST_INT test_mode = TEST_MODE_IEC_RPT;	/* test mode. default to simple test	*/
ST_RET ret;
MVL_CFG_INFO mvlCfg;
...
/* Set the ^c catcher */
  ...
  if (clientNetInfo != NULL)
	...
	/*
	getServerDirectory(m_TreeCtrl,clientNetInfo)
 //#####################################
	////여기에 넣어야된듯~!!!!!!!!!!!!!!!!!!
	void getServerDirectory(CTreeCtrl* m_TreeeCtrl, MVL_NET_INFO* clientNetInfo) {
		
	}*/
	tree *root;
	root = getNode(NULL, NULL);
	root = getServerDirectory(root, clientNetInfo);
	root = getLogicalDeviceDirectory(root, clientNetInfo);
	root = getLogicalNodeDirectory(root, clientNetInfo);
	printf("@@@@@@@@%s@@@@", root->next->name);

	getReportControlBlock(root);
    ...
    switch (test_mode)
      ...
      case TEST_MODE_IEC_RPT:
        /* NOTE: Could use mvla_getnam to get domain & RCB name from server.*/
		  ...
		  //getLogicalDeviceDirectory(clientNetInfo);

        rcb_info = test_iec_rpt (clientNetInfo, &rpt_typeids, root->next->name, "LLN0$BR$PosReport01");
...
tree* getLogicalDeviceDirectory(tree* root,MVL_NET_INFO *clientNetInfo) {
	...
	while (head->next != NULL) {
		...
		strcpy(getnam_req.dname, head->name);
		getnam_req.cont_after_pres = SD_FALSE;


		head->son = getNode(NULL, NULL);
		shead = head->son;
		...
		do {
			...
			//응답
			if (ret != SD_SUCCESS) {
				printf("\nGetLogicalDeviceDirectory failed\n");
				break;
				//printf("\n mvl_getnam () Error, ret = 0x%x.", ret);
			}
			else
			{
				printf("\nGetLogicalDeviceDirectory OK\n");
				//printf("\n mvl_getnam OK");
				getnam_resp = (NAMELIST_RESP_INFO *)reqCtrl->u.ident.resp_info;
				/*if (getnam_resp->more_follows)
					printf("\n   More Follows : TRUE");
				...
				//여기에서 트리 추가
				for (i = 0; i < getnam_resp->num_names; ++i) {
					if (strstr(nptr[i], "$") == NULL) {
						//printf("\n%s", nptr[i]);
						shead->next = getNode(nptr[i], NULL);
						shead = shead->next;
					}
					//printf("\n     %s ", nptr[i]);
				}
				if (getnam_resp->more_follows) {
					...
					strcpy(getnam_req.continue_after, nptr[i - 1]);
				}
			}
	...
	while (head->next != NULL) {
		...
		do {
			...
			else
			{
				...
				if (flag == 1) {
					if (strstr(nptr[0], last) == NULL) {
						dhead->next = getNode(last, NULL);
						dhead = dhead->next;
					}
					flag = 0;
				}
				...
				for (i = 0; i < getnam_resp->num_names; ++i) {
					if (strstr(nptr[i], "$") == NULL) {
						shead = shead->next;
						shead->son = getNode(NULL, NULL);
						dhead = shead->son;	
					}
					else if (strstr(nptr[i], "$") != NULL) {
						if (i < getnam_resp->num_names - 1) {
							if ((strstr(nptr[i + 1], nptr[i]) == NULL)) {
								dhead->next = getNode(nptr[i], NULL);
								dhead = dhead->next;
							}
						}
						else
...
tree* getReportControlBlock(tree* root) {
	...
	while (head->next != NULL) {
		...
		char* ptr=strtok(shead->name,"$");

		if (strcmp(ptr,"LLN0")==0) break;
		}
