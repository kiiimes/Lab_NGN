...
//Àº¼÷
ST_VOID test_user1_requests(MVL_NET_INFO *clientNetInfo);
ST_VOID test_user2_requests(MVL_NET_INFO *clientNetInfo);
/************************************************************************/
...
//Àº¼÷
#define TEST_MODE_USER1	5
#define TEST_MODE_USER2 6
ST_INT test_mode = TEST_MODE_USER2;
...
/* Execute Client Services ... */
    switch (test_mode)
      ...
      case TEST_MODE_IEC_REMOTE:
        ...
        break;
		//Àº¼÷
	  case TEST_MODE_USER1:
		  test_user1_requests(clientNetInfo);
		  break;
	  case TEST_MODE_USER2:
		  test_user2_requests(clientNetInfo);
		  break;
      default:
...
ST_VOID test_user1_requests(MVL_NET_INFO* clientNetInfo) {
	int type_id;
	ST_INT rdData,wrData;
	ST_RET ret;

	wrData = 12000;
	type_id = domvar_type_id_create(clientNetInfo, "E1Q1SB1C1", "CSWI2$CF$Mod$sboTimeout");
	//ret = named_var_write(clientNetInfo, "CSWI2$CF$Mod$sboTimeout", DOM_SPEC, "E1Q1SB1C1", type_id, &wrData, timeOut);
	ret = named_var_read(clientNetInfo, "CSWI2$CF$Mod$sboTimeout", DOM_SPEC, "E1Q1SB1C1", type_id, &rdData, timeOut);

	//type_id = domvar_type_id_create(clientNetInfo, "E1Q1SB1C1", "CSWI1$CF$Mod$maxVal");CSWI2$CF$Mod$sboTimeout
	//ret = named_var_read(clientNetInfo, "CSWI1$CF$Mod$maxVal", DOM_SPEC, "E1Q1SB1C1", type_id, &rdData, timeOut);
	//ret = named_var_write(clientNetInfo, "CSWI1$CF$Mod$maxVal", DOM_SPEC, "E1Q1SB1C1", type_id, &wrData, timeOut);
...
ST_VOID test_user2_requests(MVL_NET_INFO *clientNetInfo) {
	...
	if (ret != SD_SUCCESS) printf("\n MVL getname error\n");
		...
		printf("\n %dNames returned : ", getnam_resp->num_names);
	...
	while(getnam_resp->more_follows==TRUE) {
		ret = mvla_getnam(clientNetInfo, &getnam_req, &reqCtrl);
		if (ret == SD_SUCCESS) ret = waitReqDone(reqCtrl, timeOut);
		if (ret != SD_SUCCESS) printf("\n MVL getname error\n");
		else {
